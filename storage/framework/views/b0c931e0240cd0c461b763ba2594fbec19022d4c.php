<?php $__env->startSection('content'); ?>
<div class="container">
    <div class="row justify-content-center">
        <div class="center-text">
            <h1><?php echo e(__('Answer a few simple questions to get a quote.')); ?></h1>
            <p><?php echo e(__('Service at your home or office 7 days a week Fair and trasparent pricing')); ?></p>
        </div>

    <div class="col-md-12 col-sm-12 col-lg-8">
            <div class="card" id="zip-locator">
                <div class="card-header">
                    <?php if(!Auth::check()): ?>
                        <span class="bd-login-link">
                            <span><?php echo e(__('Already have an account?')); ?></span>
                            <a href="<?php echo e(route('login')); ?>" class="logueo"><?php echo e(__('log in')); ?></a>
                        </span>
                    <?php endif; ?>    
                </div>

                <div class="card-body">
                   <h3 class="g-section-header-text"><?php echo e(__('Enter your location')); ?></h3>
                   <div class="row">
                    <div class="col-md-6">
                        <form action="">
                            <div class="form-group ">
                                <label  for="zipcode">ZIP CODE</label>
                                <input type="text" id="zipcode" class="form-control" maxlength="5" >
                            </div>
                        </form>
                    </div>
                    <div class="col-md-6">
                        <div class="box-msn" style="display:none;">
                                <?php echo e(__("Great! we have certified mobile mechanics in")); ?>

                        </div>
                    </div>
                    <div class="col-md-12">
                        <a href="#" class="btn btn-primary btn-lg btn-confirmar" style="display:none;"><?php echo e(__('Confirm ZIP CODE')); ?></a>   
                    </div>
                    
                </div>                   
                </div>
            </div>

            

            <div class="card" id="tipocar" style="display:none">
                <div class="card-header"><?php echo e(__('Select your car')); ?></div>
                <div class="card-body">
                <div class="botones">
                    <div class="bd-car-selection-car-option ymake"><?php echo e(__('Make')); ?></div>
                   
                </div>
                
                    <div class="bd-car-selection-container"  id="seccioncar">
                        <div class="bd-car-selection-container" id="box1">
                            <span class="bd-car-selection-step-name"><?php echo e(__('Make')); ?></span>
                            <div class="bd-car-selection-flex-container" >
                                <ul class="bd-car-selection-flex-column ">
                                    <li class="filter">
                                        <span>A-G</span>
                                    </li>
                                   <?php $__currentLoopData = $list1; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li data-make="<?php echo e($list->name); ?>" data-id="<?php echo e($list->id); ?>"><?php echo e(ucfirst(strtolower($list->name))); ?></li>
                                   <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>     
                                </ul>

                                <ul class="bd-car-selection-flex-column">
                                    <li class="filter">
                                        <span>H-L</span>
                                    </li>
                                    <?php $__currentLoopData = $list2; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li data-make="<?php echo e($list->name); ?>" data-id="<?php echo e($list->id); ?>"><?php echo e(ucfirst(strtolower($list->name))); ?></li>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>  
                                </ul>

                                <ul class="bd-car-selection-flex-column">
                                    <li class="filter">
                                        <span>M-P</span>
                                    </li>
                                    <?php $__currentLoopData = $list3; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li data-make="<?php echo e($list->name); ?>" data-id="<?php echo e($list->id); ?>"><?php echo e(ucfirst(strtolower($list->name))); ?></li>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?> 
                                </ul>
                                <ul class="bd-car-selection-flex-column">
                                    <li class="filter">
                                        <span>R-Z</span>
                                    </li>
                                    <?php $__currentLoopData = $list4; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $list): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <li data-make="<?php echo e($list->name); ?>" data-id="<?php echo e($list->id); ?>"><?php echo e(ucfirst(strtolower($list->name))); ?></li>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?> 
                                </ul>
                            </div>
                        </div>
                        <div class="bd-car-selection-container" style="display:none" id="box2">
                                <span class="bd-car-selection-step-name"><?php echo e(__('Year')); ?></span>
                            <div class="bd-car-selection-flex-container" >
                                    <ul class="bd-car-selection-flex-column">
                                        <li class="filter">
                                            <span>2010-2107</span>
                                        </li>
                                        <?php for($i=2017; $i>=2010; $i--): ?>
                                        <li data-year="<?php echo e($i); ?>"><?php echo e($i); ?></li>
                                        <?php endfor; ?>
                                    </ul>
    
                                    <ul class="bd-car-selection-flex-column">
                                        <li class="filter">
                                            <span>2004-2009</span>
                                        </li>
                                        <?php for($i=2009; $i>=2004; $i--): ?>
                                        <li data-year="<?php echo e($i); ?>"><?php echo e($i); ?></li>
                                        <?php endfor; ?>
                                    </ul>
                                    <ul class="bd-car-selection-flex-column">
                                            <li class="filter">
                                                <span>2001-2004</span>
                                            </li>
                                            <?php for($i=2004; $i>=2001; $i--): ?>
                                            <li data-year="<?php echo e($i); ?>"><?php echo e($i); ?></li>
                                            <?php endfor; ?>
                                    </ul>
    
                                   
                            </div>
                        </div>
                        <div class="bd-car-selection-container" style="display:none" id="box3">
                                <span class="bd-car-selection-step-name"><?php echo e(__('Model')); ?></span>
                            <div class="bd-car-selection-flex-container" id="box3">
                                    <ul class="bd-car-selection-flex-column" id="mimodelo">
                                       
                                        
                                        <li>ILX</li>
                                       
                                    </ul>
                                    <ul class="bd-car-selection-flex-column" id="mimodelo2">
                                       
                                        
                                        <li></li>
                                       
                                    </ul>
                                    <ul class="bd-car-selection-flex-column" id="mimodelo3">
                                       
                                        
                                        <li></li>
                                       
                                    </ul>
                                    <ul class="bd-car-selection-flex-column" id="mimodelo4">
                                       
                                        
                                        <li></li>
                                       
                                    </ul>
                            </div>

                        </div>

                    </div>
                </div>
            </div>


      <div class="card" id="services" style="display:none">
                    <div class="card-header"><?php echo e(__('Select your Services')); ?></div>
                    <div class="card-body">

                        <div class="service-selection">
                          <div class="bd-service-tabs-container">
                            <div class="bd-service--tab bd-service--tab__active">
                              <h4><?php echo e(__('REPAIR & MAINTENANCE SERVICES')); ?></h4>
                            </div>
                            <div class="bd-service--tab ">
                                    <h4 class="respuestas"></h4>
                            </div>                                      
                          
                        </div>   
                            <form class="cd-form floating-labels js" >
                                <div class="form-group" >
                                    <label for="cd-search" ><?php echo e(__('search')); ?></label>
                                    <input class="search form-control" id="cd-search" type="text" autocomplete="off" value="">
                                </div>

                                <div class="bd-service-list-container">
                                    <div class="bd-service-list bd-service-list--categories">
                                        <div class="bd-service-list--items">
                                            <ul>
                                                <?php $__currentLoopData = $servicios; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $service): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                                <li data-id="<?php echo e($service->id); ?>"><?php echo e($service->name); ?></li>
                                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                            </ul>
                                        </div>
                                    </div>

                                    <div class="bd-service-list bd-service-list--services">
                                        <div class="bd-service-list--category">
                                            <?php echo e(__('Popular services')); ?>

                                        </div>
                                        <div id="contsubs">
                                            
                                           
                                            
                                        </div>
                                        
                                    </div>
                                </div>
                            </form>

                        </div>
                    </div>
            </div>

            <div class="card" id="detalle" style="display:none">
                <div class="card-header">
                        <h3 class="g-section-header-text pull-left"><?php echo e(__('Enter your location')); ?></h3>
                        <div class="bd-service-options-container pull-right" >
                            <div class="bd-service-list__details" >Details</div>
                            <div class="bd-service-list__delete" >
                                <img src="data:image/svg+xml;base64,">
                            </div>
                            <div class="clearfix"></div>
                        </div>
                </div>
                <div class="card-body">
                        <div class="mainblue-info-box"><img src="data:image/svg+xml;base64," class="bd-guarantee-icon">
                            <p>
                                <span><?php echo e(__('Your service is backed by our 12-month, 12,000-mile warranty.')); ?> </span>
                            </p>
                        </div>
                        <div class="col-md-4">
                            <button type="button" id="btnservice" class="btn-orange-fill">Continue</button>
                        </div>
                </div>
            </div>


            <div class="card" id="appoint" style="display: none">
                <div class="card-header">
                        <?php echo e(__('Select the time of your appointment')); ?>

                </div>
                <div class="card-body">
                    <div class="bd-time-container">
                        <div class="row">
                            <div class="col-md-12">
                                <div class="green-info-box">
                                    <div class="exclamation-circle">!</div>
                                    <p>Youre just in time! Our earliest available times are likely to be sold out soon.</p>
                                </div>
                            </div>
                        </div>

                        <div class="bd-time--header" >
                                <div class="bd-time--header--innerpadding">
                                   <div class="bd-time--header__value">
                                      <div class="row-flex-centered">
                                         <div class="col-sm-6">
                                            <p>Estimated service time:&nbsp;</p>
                                            <h5 ><span>45 mins</span>
                                                <span>&nbsp;</span></h5>
                                            <div class="bd-small-circle-tooltip">
                                               <div class="bd-tooltip-est">Time required for all services to be done.</div>
                                               <span>?</span>
                                            </div>
                                         </div>
                                      </div>
                                   </div>
                                </div>
                        </div>

                        <!--horas-->

                        <div class="bd-time--zone" >
                                <div>
                                   <div class="row bd-time-earliest row-flex-centered">
                                      <div class="col-sm-6">
                                         <p>Earliest Available Time</p>
                                         <h5>
                                            <div><span>Tomorrow, </span><time datetime="2018-08-29T00:00:00-05:00">07:00 AM</span></div>
                                         </h5>
                                      </div>
                                      <div class="col-sm-6">
                                          <button type="button" class="btn-orange-fill">Select Earliest Time</button>
                                      </div>
                                   </div>
                                   <div class="row bd-time-earliest__spacing">
                                      <div class="col-xs-12 col-sm-12 text-center">
                                         <div class="bd-optional-services__text bd-time-earliest__text">or choose a different time</div>
                                         <div class="bd-optional-services__line"></div>
                                      </div>
                                   </div>
                                </div>
                                <div class="spacet-20">
                                   <div class="bd-time--zone__single">
                                      <div class="bd-time--zone__day">
                                         <div class="appointment-date-header"></div>
                                         <h5><time datetime="2018-08-29T00:00:00-05:00"><?php echo e(date("l, F d")); ?></time></h5>
                                      </div>
                                      <div class="bd-timeline">
                                         <div class="slotlist">
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot" ></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot" ></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot" ></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot" ></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot" ></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot "></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot "></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                            <div class="wrapper">
                                               <div class="slot"></div>
                                            </div>
                                         </div>
                                      </div>
                                      <div class="bd-timeline__hours">
                                         <ul>
                                            <li>7AM</li>
                                            <li>9AM</li>
                                            <li>11AM</li>
                                            <li>1PM</li>
                                            <li>3PM</li>
                                            <li>5PM</li>
                                            <li>7PM</li>
                                            <li>9PM</li>
                                         </ul>
                                      </div>
                                   </div>
                                  
                                 
                                </div>
                             </div>
                        
                
                        </div>

                    </div>
            </div>

        <section class="bd-section bd-section-mechanic-notes bd-section--active bd-section-mechanic-notes--active" id="notes" style="display: none">
                <div class="shadow-box-fill bd-box-padding" >
                    <div class="row" >
                        <div class="col-xs-12 col-md-12" >
                            <h3 class="g-section-header-text1 pull-left" >Notes for the mechanic</h3>
                            <div class="bd-service-options-container pull-right">
                                <div class="bd-service-list__delete" >
                                    <img src="data:image/svg+xml;base64,">
                                </div>
                                <div class="clearfix" >
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="row">
                            <div class="col-xs-12 col-md-12">
                                <div class="cd-form floating-labels spacet-10">
                                    <div class="no-icon">
                                        <textarea class="small-textarea" placeholder="Add any notes that may be useful for the mechanic to know."></textarea>
                                    </div>
                                </div>
                               
                            </div>
                        </div>
                    </div>
            </section>
           
            <section class="row bd-section bd-section--payment bd-section" id="fr-contact" style="display:none" >
                    <div class="col-xs-12 col-md-12" >
                       <div class="shadow-box-fill bd-box-padding" >
                          <h3 class="g-section-header-text1">Your contact information</h3>
                          <div class="row spacet-10">
                             <div class="col-sm-6">
                                <div class="cd-form floating-labels js" >
                                   <div class="icon" >
                                       <label for="contactName" class="cd-label" >Full name</label>
                                       <input id="contactName" class="user" type="text" value="" >
                                    </div>
                                </div>
                             </div>
                             <div class="col-sm-6" >
                                <div class="cd-form floating-labels js" >
                                   <div class="icon" >
                                       <label for="contactEmail" class="cd-label">Email</label>
                                       <input id="contactEmail" class="email" type="text" value="">
                                    </div>
                                </div>
                             </div>
                          </div>
                          <div class="row spacet-10">
                             <div class="col-sm-6">
                                <div class="cd-form floating-labels js">
                                   <div class="icon">
                                       <label for="contactPhone" class="cd-label">Phone number</label>
                                       <input id="contactPhone" class="phonenumber" type="text" value=""></div>
                                </div>
                             </div>
                          </div>
                       </div>

                       <div class="row">
                           <div class="col-sm-5 col-centered">
                               <div class="btn-orange-fill spacet-20">Continue</div>
                            </div>
                        </div>
                    </div>
                 </section>

    </div>
<!-- side-->
    <div class="col-lg-4 hidden-sm hidden-md">
                <div class="map" id="canvas">
                                   
                </div>
            <div class="card">
                <div class="card-header bd-sidebar-price-box">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="bd-sidebar-price-box__subtitle">
                                <?php echo e(__('Your total price')); ?>

                            </div>
                            <div class="bd-sidebar-price-box__price">
                                $ <span>0.00</span>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="bd-sidebar-price-box__saving">
                                <?php echo e(__('Please select a service')); ?>

                            </div>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                    <div class="col-xs-12">
                        <h3 class="g-section-header-text"><?php echo e(__('Summary')); ?></h3>
                        <div id="sidebar-zip" class="bd-sidebar-property bd-sidebar-property--scrollable spacet-10">
                            30301 - atlanta
                        </div>
                        <div id="sidebar-car" class="bd-sidebar-property bd-sidebar-property--scrollable">
                            Select car
                        </div>
                    </div>
                    <div class="col-xs-12">
                        <div class="bd-optional-services">
                            <div class="bd-optional-services__text">
                                <?php echo e(__('Services')); ?>

                            </div>
                            <div class="bd-optional-services__line">
                               
                            </div>
                        </div>
                    </div>
                    <div class="col-xs-12">
                        <div class="bd-sidebar-service-list__item">
                            <div class="bd-service-list__toggle pull-left">
                                    <div class="btn-toggle mb-summary-btn-toggle">
                                            <input class="tgl tgl-blue" type="checkbox" id="toggle0">
                                            <label class="tgl-btn bd-sidebar-service-list__toggle--small"></label>
                                    </div>
                                    
                            </div>

                            <div class="bd-sidebar-property bd-sidebar-job-name bd-sidebar-property--scrollable pull-left">
                                    Change Oil and Filter
                            </div>
                            <div class="bd-service-list__delete pull-right" >
                                    <img src="data:image/svg+xml;base64,">
                                </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<form id="datatool">
    <?php echo csrf_field(); ?>
</form>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.app', array_except(get_defined_vars(), array('__data', '__path')))->render(); ?>